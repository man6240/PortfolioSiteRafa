...
/* A featured project staged like a product shot: the phone floats at the centre of a dark
   scene in the game's own colours, big words sit around it, crop marks frame the panel, and
...
export default function Scene({ p, index, total, onOpen }) {
  ...
  const { words, tags, colors } = p.scene;
  ...
  return (
    <article className="scene reveal" style={vars} ref={root}>
      ...
      {['tl', 'tr', 'bl', 'br'].map((c) => <span key={c} className={`crop ${c}`} aria-hidden="true" />)}
      ...
      <button className="scene-hero" onClick={() => onOpen(p, 0)} aria-label={`Open ${title} gallery`}>
        <span className="scene-float"><PhoneFrame src={p.shots[0]} /></span>
      </button>
